...
3. Production integration: card classification and strategic dispatch now read from the registry. An opt-in payment-directed coverage path is available through `RALGORITHM_PAYMENT_DIRECTED=1`; full packed-state generation and a faster cost-specific payment planner remain the next migration stages.
...
The current generation comparison is documented in [docs/generation4_benchmark.md](docs/generation4_benchmark.md).
